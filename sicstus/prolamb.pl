#! /var/task/bin/sicstus

:- set_prolog_flag(verbose, silent).

:- initialization go.

:- module(library(pairs)).
:- module(library(http/json)).
:- module(library(http/http_json)).
:- module(library(apply), [maplist/3]).
:- module(library(http/http_client), [http_get/3, http_post/4]).
